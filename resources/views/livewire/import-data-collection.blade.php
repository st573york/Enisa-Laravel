<div wire:poll.3000ms>
    @php
        ...
        if ($task)
        {
            ...
            if ($task->status->id != 1) {
                $last_import_date = (isset($task->payload['last_import_data_collection_at'])) ? $task->payload['last_import_data_collection_at'] : $last_import_date;
            }
        }

        // Fire task events
        $this->emit('importDataCollection' . preg_replace('/[\s_]/', '', $task_name));
    @endphp
        ...
        <div class="col-10 offset-1 ps-0 pe-0">
            <div class="in-progress-wrap d-none {{ ($task && $task->status->id == 1) ? 'show' : '' }}">
                <section class="in-progress-section ps-4 pe-4">
                    <div class="row h-100 align-items-center">
                        <div class="col-md-12">
                            <span>{{ __('Import data in progress. Please wait...') }}</span>
                        </div>
                    </div>
                </section>
            </div>
            <div class="approved-wrap d-none {{ ($task && $task->status->id == 2) ? 'show' : '' }}">
                <section class="approved-section ps-4 pe-4">
                    <div class="row h-100 align-items-center">
                        <div class="col-md-12">
                            <span class="svg-approved">{{ __('Import data completed.') }}</span>
                        </div>
                    </div>
                </section>
            </div>
            <div class="failed-wrap d-none {{ ($task && $task->status->id == 3) ? 'show' : '' }}">
                <section class="failed-section ps-4 pe-4">
                    <div class="row h-100 align-items-center">
                        <div class="col-md-12">
                            <span  class="svg-failed">{{ __('Import data failed.') }}</span>
                        </div>
                    </div>
                </section>
            </div>
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-lg-6 col-xl-6">
                        <div class="d-flex gap-2 justify-content-end ps-0 pe-0">
                            <label for="last_import_date" class="col-form-label mt-1">Last import date:</label>
                            <div class="col-5 mt-1">
                                <div class="input-group date">
                                    <input type="text" class="form-control datepicker" id="last_import_date" value="{{ $last_import_date }}" disabled/>
                ...
                <div class="row mt-5">
                    <div class="col-12 col-lg-6 col-xl-6">
                        <div class="d-flex gap-3">
                            <button type="button" class="btn btn-enisa import-data"
                                {{ (($task && $task->status->id == 1) || !$is_latest_index) ? 'disabled' : '' }}>{{ __('Import Data') }}
                            </button>
                            <span>{{ __('Import status') }}:<br />
                    ...
                    <div class="col-12 col-lg-6 col-xl-6">
                        <div class="d-flex gap-2 justify-content-end ps-0 pe-0">
                            ...
                            <span wire:ignore class="download-section" item-id="download-import-data-collection">
