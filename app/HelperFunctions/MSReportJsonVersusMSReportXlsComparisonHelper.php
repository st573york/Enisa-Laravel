...
class MSReportJsonVersusMSReportXlsComparisonHelper
{
    ...
    private static function validateOverviewSheetHeader($country, &$rows, &$xls_errors)
    {
        ...
        if (FilesComparisonHelper::valuesDiff($header[4], $country->iso)) {
            $xls_errors[self::$active_sheet]['Header']['E1'] = FilesComparisonHelper::formatResult($country->iso, $header[4]);
        }
        if (FilesComparisonHelper::valuesDiff($header[5], 'Difference')) {
            $xls_errors[self::$active_sheet]['Header']['F1'] = FilesComparisonHelper::formatResult('Difference', $header[5]);
        }
        if (FilesComparisonHelper::valuesDiff($header[6], 'Speedometer')) {
            $xls_errors[self::$active_sheet]['Header']['G1'] = FilesComparisonHelper::formatResult('Speedometer', $header[6]);
        }
        if (FilesComparisonHelper::valuesDiff($header[7], 'Reference Year')) {
            $xls_errors[self::$active_sheet]['Header']['H1'] = FilesComparisonHelper::formatResult('Reference Year', $header[7]);
        }
        if (FilesComparisonHelper::valuesDiff($header[8], 'Source')) {
            $xls_errors[self::$active_sheet]['Header']['I1'] = FilesComparisonHelper::formatResult('Source', $header[8]);
        }
    }
    ...
    private static function compareOverviewSheetWithJsonValues($json_data, $excel_item, &$xls_errors)
    {
        ...
        $val = 3;
        foreach (FilesComparisonHelper::$ms_report_xls_cols as $col)
        {
    ...
    private static function validatePerformingIndicatorsSheetHeader($country, &$rows, &$xls_errors)
    {
        $header = array_shift($rows);
        ...
        if (FilesComparisonHelper::valuesDiff($header[6], 'Difference')) {
            $xls_errors[self::$active_sheet]['Header']['G1'] = FilesComparisonHelper::formatResult('Difference', $header[6]);
        }
        if (FilesComparisonHelper::valuesDiff($header[7], 'Speedometer')) {
            $xls_errors[self::$active_sheet]['Header']['H1'] = FilesComparisonHelper::formatResult('Speedometer', $header[7]);
        }
        if (FilesComparisonHelper::valuesDiff($header[8], 'Source')) {
            $xls_errors[self::$active_sheet]['Header']['I1'] = FilesComparisonHelper::formatResult('Source', $header[8]);
        }
    ...
    public static function validateAndComparePerformingIndicatorsSheet($country, $json_data, $sheet, &$xls_errors, $indicators_type)
    {
        ...
        self::validatePerformingIndicatorsSheetHeader($country, $rows, $xls_errors);
